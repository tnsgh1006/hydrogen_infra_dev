...
import { isIntrinsicType, isSymbolFlagSet, isTypeReference, hasType, isIntrinsicErrorType, isUnionType, isIntersectionType, isConditionalType, isObjectType, isPropertyReadonlyInType } from 'ts-api-utils';
                ...
                if (this.m_data.alias === null) {
                    ...
                    if (aliasType.aliasTypeArguments === undefined) {
                        ...
                        const aliasDeclaration = (aliasDeclarations?.length ?? 0) === 1
                            ? aliasDeclarations[0]
                            : undefined;
                        const aliasTypeNode = aliasDeclaration !== undefined && hasType(aliasDeclaration)
 ...
 */
function getTypeImmutability(program, typeOrTypeNode, overrides = getDefaultOverrides(), useCache = true, maxImmutability = Immutability.Immutable) {
...
function arrayImmutability(program, typeData, overrides, cache, maxImmutability) {
    ...
    if (shallowImmutability <= Immutability.ReadonlyShallow ||
    ...
    return clamp(Immutability.ReadonlyShallow, deepImmutability, shallowImmutability);
}
/**
...
function objectImmutability(program, typeData, overrides, cache, maxImmutability) {
    ...
    if (properties.length > 0) {
        ...
        for (const property of properties) {
            if (isPropertyReadonlyInType(typeData.type, property.getEscapedName(), checker) ||
        ...
        // eslint-disable-next-line functional/no-loop-statements
        for (const property of properties) {
            const propertyType = getTypeOfPropertyOfType(checker, typeData.type, property);
...
function indexSignatureImmutability(program, typeData, kind, overrides, cache, maxImmutability) {
    ...
    if (indexInfo.isReadonly) {
        ...
        return max(Immutability.ReadonlyShallow, getTypeImmutabilityHelper(program, getTypeData(indexInfo.type, undefined), // TODO: can we get a type node for this?
        overrides, cache, maxImmutability));
    }
    return Immutability.Mutable;
}
